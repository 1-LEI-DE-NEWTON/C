...
int main(int argc, char **argv){
    ...
    {      
        ...
        {                        
            ...
            scanf("%s", tratarFiltro);
            //Consome o caractere de nova linha
            getchar();
            
            /*
            Aloca o tamanho da variavel tratarFiltro na variavel ponteiroFiltro            
            O +1 é para alocar o caractere de fim de string
            */
            
            ponteiroFiltro = realloc(ponteiroFiltro, strlen(tratarFiltro) + 1);
            //Copia o conteudo da variavel tratarFiltro para a variavel ponteiroFiltro            
            strcpy(ponteiroFiltro, tratarFiltro);
            printf("\nFiltro (Ativo): %s\n", ponteiroFiltro);
            exibirTopoTabela();
        }
        if (respostaFiltro == 'q')
        {
            printf("Informe a quantidade que deseja filtrar (Coberto, Travado ou Descoberto):");            
            printf("Digite a quantidade, apenas números: ");
            scanf("%s", tratarFiltro);
            //Consome o caractere de nova linha
            getchar();
            //Copia o conteudo da variavel tratarFiltro para uma variavel auxiliar numérica
            int quantidadeFiltro = atof(tratarFiltro);
            //Aloca o tamanho da variavel quantidadeFiltro na variavel ponteiroFiltro
            ponteiroFiltro = realloc(ponteiroFiltro, sizeof(quantidadeFiltro));
            //Copia o conteudo da variavel quantidadeFiltro para a variavel ponteiroFiltro
            strcpy(ponteiroFiltro, tratarFiltro);
            printf("\nFiltro (Quantidade): %s\n", ponteiroFiltro);
            exibirTopoTabela();
        }
    ...
    {                
        ...
        */       
        ...
        {                        
            ...
            if (strcmp(ponteiroFiltro, instruments[2]) == 0)
            {
                printf("| %-15s ", derivatives[0]);
                printf("|  %-10s | %4s | %-10s ", derivatives[1], instruments[2], instruments[7]); 
                printf("| %6s | %4s ", instruments[35], instruments[36]);
        ...
        else if (resposta == 'e' && respostaFiltro == 'q')
        {
            ...
            double filtroQuantidade = atof(ponteiroFiltro);
